...
void teller_thread(BankAccount* b, int _amount) {
    // TODO: call transaction() function for BankAccount b
...
void teller_threadsafe(BankAccount* c, int _amount) {
    ...
    // use appropriate transaction handler
...
int main(int argc, char* argv[]) {
    ...
    // loop over the trans_array
    for(int i = 0; i < num_trans; i++){
        // create a thread for each element in array
        t_vec.push_back(thread(teller_thread, &B, trans_arr[i])); //element in for loop, for not is hardcoded 0
    }

    // (already complete) join threads after going through for loop
    for (size_t i = 0; i < t_vec.size(); ++i) {
        t_vec.at(i).join(); //join does what wait does for processes
    }
    // end timer and print result
    ...
    for(int j = 0; j < num_trans; j++){
        t_vec2.push_back(thread(teller_threadsafe, &C, trans_arr[j]));
    }

    // TODO: join all threads in t_vec2
    for (size_t k = 0; k < t_vec2.size(); k++) {
        t_vec2.at(k).join(); //join does what wait does for processes
    }
