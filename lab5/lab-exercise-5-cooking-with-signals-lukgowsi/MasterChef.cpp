...
static void timerHandler( int sig, siginfo_t *si, void *uc )
{
	...
	comp_item->PrintComplete(); //print the step is complete
	completedSteps->push_back(comp_item->id); //add the complete step to the completedSteps vector
	completeCount += 1; //update completed count
...
int main(int argc, char **argv)
{
	...
	sigaction(SIGRTMIN, &sa, NULL); //changes what happens when SIGRTMIN signal is raised
	...
	while(recipeSteps->Count() != completeCount){ //check if completed steps is same as steps in recipeSteps
		vector<Step*> ready = recipeSteps->GetReadySteps(); //get a vector of the steps ready to be run
		for(size_t i = 0; i < ready.size(); i++){ //run the steps that are be ready to be run
			makeTimer(ready[i], ready[i]->duration);
			ready[i]->running = true;
		}
	}
	/* End Section - 1 */
