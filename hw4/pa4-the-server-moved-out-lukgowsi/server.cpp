...
int main (int argc, char* argv[]) {
	...
	while (true) {
		...
		//returns a connection port to the channel
		connection = control_channel->accept_conn();

		//make a new channel to write with channel
		TCPRequestChannel* writeChan = new TCPRequestChannel(connection);

		//makes a thread by reading from a channel to handle the processes
		thread writeThread(handle_process_loop, writeChan);

		//use detach to not wait for thread
		writeThread.detach();
	}
